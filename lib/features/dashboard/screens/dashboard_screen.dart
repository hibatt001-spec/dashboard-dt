...
class _DashboardScreenState extends State<DashboardScreen> {
    ...
    return Container(
      ...
      /*child: Row(
        ...
        children: [
          ...
          Row(
            children: [
              _statusDot(t('system_ok'), successGreen),
              const SizedBox(width: 16),
              _statusDot(t('mqtt_ok'), neonCyan),
  ...
  void _handleLogout(String Function(String) t, ThemeMode, [Color? mainText] ) {
    showDialog(
      context: context,
      builder: (ctx) {
        ...
        return AlertDialog(
          backgroundColor: isDark ? const Color(0xFF182338) : Colors.white,
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(12),
            side: BorderSide(color: isDark ? const Color(0xFF2E3F5F) : const Color(0xFFE2E8F0)),
          ),
          title: Text(
            t('logout_title'),
          ...
          actions: [
            ...
            TextButton(
              onPressed: () async {
                ...
                // قطع اتصال الـ MQTT لحماية السيرفر
...
// ════════════════════════════════════════════════════════════
  //  STATE LIFECYCLE (INIT, DISPOSE & DEPENDENCIES)
  // ════════════════════════════════════════════════════════════
  @override
  void initState() {
    super.initState();

    // 📡 تهيئة الـ MQTT Service وربط الـ Callbacks
    _mqttService = MqttService(
      onTelemetryReceived: (temp, vib, current) {
        if (!mounted) return;
        setState(() {
          ...
          // حساب الـ RPM تقريبياً لمحرك SEW-EURODRIVE
          if (current > 0.5) {
            currentRPM = (1480 - (current * 5)).clamp(0, 1500);
          } else {
            currentRPM = 0.0;
          }
        });
      }, // 👈 إغلاق الـ onTelemetryReceived
      onStatusChanged: (isConnected) {
        if (!mounted) return;
        setState(() {
          isSimulinkConnected = isConnected;
        });
      }, // 👈 إغلاق الـ onStatusChanged
    ); // 👈 إغلاق الـ Constructor الخاص بالـ Service

    // 🚀 بدء الاتصال بالـ Broker فوراً
    _mqttService.connect();
  } // 👈 إغلاق دالة الـ initState بنجاح (هذا القوس الذي كان يسبب المشكلة!)

  @override
  void dispose() {
    // قطع اتصال الـ MQTT لحماية السيرفر والموارد
    _mqttService.disconnect();
    super.dispose();
  } // 👈 إغلاق دالة الـ dispose بنجاح
  ...
  } // 👈 إغلاق دالة الـ didChangeDependencies بنجاح
